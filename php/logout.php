...
if (isset($_SESSION['unique_id'])) { // if the user is logged in
    ...
    $logout_id = mysqli_real_escape_string($conn, $_GET['logout_id']); // get the ID of the user to log out

    if (isset($logout_id)) { // if the user ID is provided
        $status = "Offline now"; // set the user's status to offline
        $sql = mysqli_query($conn, "UPDATE users SET status = '{$status}' WHERE unique_id={$_GET['logout_id']}"); // update the user's status in the database
