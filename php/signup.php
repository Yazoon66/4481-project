...
if (!empty($fname) && !empty($lname) && !empty($email) && !empty($password)) {
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if (mysqli_num_rows($sql) > 0) {
            //this states if the email address already exist then tell that to the user
            echo "$email - this email already exist! ";
        } else {
            //in this else statment we will check if the user upplad a filer or not (image I mean)
            if (isset($_FILES["image"])) {
                ...
                if (in_array($img_ext, $extensions) === true) {
                    //this is if the user is uploaded img ext is matched with array extensions
